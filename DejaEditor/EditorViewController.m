...
#import "APIDocumentViewController.h"
...
@interface EditorViewController () <UITextViewDelegate, UITableViewDataSource, UITableViewDelegate, QRFindReplaceViewDelegate>
...
@property(nonatomic, retain)id<MethodFinder> methodFinder;
@property(nonatomic, retain)TextInputCatcher *textInputCatcher;
...
@property(nonatomic, retain)TextInputCatcher *findTextInputCatcher;
...
- (void)viewDidLoad
{
    ...
    self.methodFinder = [MethodFinderFactory methodFinderWithProject:self.project scriptName:self.scriptName];
    [self startAnalyse];
    self.textInputCatcher = [[[TextInputCatcher alloc] initWithWaitingInterval:0.50f] autorelease];
...
- (void)viewAPIButtonTapped:(UIBarButtonItem *)barButtonItem
{
    ...
    if(!self.docViewController){
        APIDocumentViewController *docVC = [[APIDocumentViewController new] autorelease];
...
- (NSArray *)stringPositionListWithText:(NSString *)text matching:(NSString *)matching
{
    ...
    while((beginIndex = [text find:matching fromIndex:beginIndex]) != -1){
        [list addObject:[StringPosition createWithPosition:beginIndex string:matching]];
        ++beginIndex;
    }
...
- (void)pasteButtonTapped
{
    ...
    [allPositionList sortUsingComparator:^NSComparisonResult(StringPosition *obj1, StringPosition *obj2) {
        return obj1.position > obj2.position ? NSOrderedDescending : NSOrderedAscending;
    }];
    NSInteger lastIndex = 0;
    for(NSInteger i = 0; i < allPositionList.count; ++i){
        StringPosition *sp = [allPositionList objectAtIndex:i];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(tableView == self.pretypeSelectionListTableView){
        NSString *text = self.textView.text;
        NSString *prefix = nil;
        Pretype *tmpPretype = [self.pretypeSelectionList objectAtIndex:indexPath.row];
        ...
        FunctionPosition *tmpFP = [self.functionPositionList objectAtIndex:indexPath.row];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if(tableView == self.pretypeSelectionListTableView){
        ...
        FunctionPosition *fp = [self.functionPositionList objectAtIndex:indexPath.row];
...
- (void)QRFindReplaceView:(QRFindReplaceView *)view findTextFieldTextDidChange:(NSString *)text
{
    if(!self.findTextInputCatcher){
        self.findTextInputCatcher = [[[TextInputCatcher alloc] initWithWaitingInterval:0.50f] autorelease];
...
{
    StringPosition *sp = [self.stringPositionFinder previousPosition];
    if(sp){
        self.textView.selectedRange = NSMakeRange(sp.position, sp.string.length);
        [self.textView becomeFirstResponder];
        [self.textView scrollRangeToVisible:self.textView.selectedRange];
...
{
    StringPosition *sp = [self.stringPositionFinder nextPosition];
    if(sp){
        self.textView.selectedRange = NSMakeRange(sp.position, sp.string.length);
        [self.textView becomeFirstResponder];
        [self.textView scrollRangeToVisible:self.textView.selectedRange];
