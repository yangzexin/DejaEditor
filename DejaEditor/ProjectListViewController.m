...
- (void)viewDidLoad
{
    ...
    self.projectManager = [ProjectManagerFactory defaultProjectManager];
    ...
    if([[NSUserDefaults standardUserDefaults] objectForKey:@"projectImported"] == nil){
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            for(NSString *demoProjectName in demoProjectNameList){
                ...
                if(![ZipProjectManager zipProjectExistsWithName:demoProjectName]){
                    [ZipProjectManager importZipWithContentOfFile:tmpPath];
...
- (void)reloadProjectList
{
    ...
    self.projectArchiveList = [ZipProjectManager zipProjectNameList];
...
- (NSString *)unzipProjectWithName:(NSString *)zipProjectName
{
    ...
    [ZipProjectManager unzipProjectWithName:zipProjectName toDirectoryPath:tmpFatherPath];
...
- (BOOL)copyZipProjectToProjectLibraryFromDirectoryPath:(NSString *)directoryPath
{
    ...
    if(![self.projectManager projectExistsWithName:projectName]){
        id<Project> project = [self.projectManager projectWithName:projectName];
...
- (void)archiveButtonTapped
{
    ...
    [ZipProjectManager zipProject:[self.projectManager projectWithName:[self.projectList objectAtIndex:indexPath.row]]];
    [self reloadProjectList];
}
...
{
    NSIndexPath *indexPath = [self.tableView indexPathForSelectedRow];
    [ZipProjectManager packageProject:[self.projectManager projectWithName:[self.projectList objectAtIndex:indexPath.row]]];
    [self reloadProjectList];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(indexPath.section == 0){
        id<Project> project = [self.projectManager projectWithName:[self.projectList objectAtIndex:indexPath.row]];
        ProjectViewController *projectVC = [[[ProjectViewController alloc] initWithProject:project] autorelease];
    ...
    if(editingStyle == UITableViewCellEditingStyleDelete){
        if(indexPath.section == 0){
            ...
            [AlertDialog showWithTitle:[NSString stringWithFormat:@"确定要删除项目%@吗", zipProjectName]  message:nil completion:^(NSInteger buttonIndex, NSString *buttonTitle) {
                if(buttonIndex == 1){
                    [ZipProjectManager removeZipProjecWithName:zipProjectName];
                    self.projectArchiveList = [ZipProjectManager zipProjectNameList];
                    [tableView beginUpdates];
                    [tableView deleteRowsAtIndexPaths:[NSArray arrayWithObject:indexPath] withRowAnimation:UITableViewRowAnimationAutomatic];
                    [tableView endUpdates];
