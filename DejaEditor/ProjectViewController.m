...
#import "LinkProjectViewController.h"
...
- (void)runProjectWithMainScriptName:(NSString *)mainScriptName
{
    ...
    ProjectBundle *bundle = [[[ProjectBundle alloc] initWithProject:self.project] autorelease];
...
- (void)editScriptWithName:(NSString *)scriptName
{
    ...
    EditorViewController *editorVC = [[[EditorViewController alloc] initWithProject:self.project
...
- (void)linkButtonTapped
{
    ...
    LinkProjectViewController *selectProjectVC = [[[LinkProjectViewController alloc] initWithProjectNameList:projectNameList
    ...
    if(!tableView.isEditing){
        if(indexPath.section == 0){
            ...
            [AlertDialog showWithTitle:@"选择已有项目打开文件" message:nil completion:^(NSInteger buttonIndex, NSString *buttonTitle) {
                if(buttonIndex != 0){
                    [[LogView sharedInstance] clear];
                    id<Project> selectedProject = [self.projectManager projectWithName:[projectNameList objectAtIndex:buttonIndex - 1]];
                    ProjectBundle *bundle = [[[ProjectBundle alloc] initWithProject:selectedProject] autorelease];
                    LINavigationController *nc = [[LINavigationController new] autorelease];
                    [nc setStopButtonTapBlock:^{
                        [self stopAllRunningAppAnimated:NO];
                    }];
